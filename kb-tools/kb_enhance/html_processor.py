...
class HTMLProcessor:
    ...
    def extract_kb_article(self, article_path: str, debug: bool = True) -> Dict[str, str]:
        """
        Extract and clean content from a KB article.

        Args:
            article_path: Path to the KB article file.
            debug: Whether to save debug information to files. This parameter is kept for backward compatibility,
                   but the text debug file creation has been removed in favor of markdown debug files.
        ...
        try:
            ...
            if not content_elem:
                ...
                content_selector_used = "body" if content_elem.name == "body" else "soup"
            
            # Save a copy of the content element before cleaning
            original_content_elem = content_elem.__copy__()
            
            # Remove unwanted elements
            unwanted_elements = ["script", "style", "nav", "header", "footer", 
                "aside", "iframe", "form", "button", "input",
                "select", "textarea", "noscript"]
            
            removed_elements = []
            for element in content_elem(unwanted_elements):
                element.decompose()
                removed_elements.append(element.name)
            ...
            for elem in content_elem.find_all(['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'p', 'li']):
                if elem.name.startswith('h'):
                    ...
                    processed_elements.append(f"{elem.name}: {elem.get_text(strip=True)}")
                else:
            ...
            # The text debug file is no longer created
            # Debug parameter is kept for backward compatibility
