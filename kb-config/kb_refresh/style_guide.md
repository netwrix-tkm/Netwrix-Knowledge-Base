...
* Commands to be used
* URLs and paths (however, if a URL or a path needs to be on the same line as another part of the sentence, use <code> tags)

Code blocks are recommended for when the contents are to be copied. Example:

```markup
<pre class="has-inner-focus">
This is a code block. 
Note that actual Enter\Return line breaks work here as intended. 
For example, all line breaks are created with no "<br>" tags. 
...
```markup
<code class="lang-Registry">This is a normal non-copyable code block.</code>
```
...
5. **If in a list, surround with** `<br>` **tags.** Add space around an image if it is part of a list to make sure it renders properly in the published article.
